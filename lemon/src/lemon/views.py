...
from api.models import TIME_SLOTS, Booking, Meal
    ...
    def post(self, request):
        ...
        if booking.is_valid:
            ...
            if all_guests_count > TOTAL_AVAILABLE_PLACES:
                return JsonResponse(
                    {
                        "message": f"Fail - Dear {first_name} {last_name}, unfortunately we do not have enough space capacity on {reservation_date} at {reservation_time} for {guest_count} people! Please try to choose another date and time!"
                    }
                )
            ...
            return JsonResponse(
                {"message": f"Success - Booking for {first_name} {last_name} completed!"}
            )
        return self.render_new_booking(request, booking)
...
def menu_item(request, pk):
    try:
        ...
        print(f"Meal object doesn't exist for pk = {pk}")
        item = ""
    item_dict = {"item": item}
