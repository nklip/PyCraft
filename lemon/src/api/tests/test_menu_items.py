...
class MenuItemsTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.user = User.objects.create_user(username="manager@manager.com", password="manager")
    ...
    def test_menu_items_as_customer(self):
        ...
        self.assertEqual(response.status_code, 403)
        # delete
        response = self.client.delete(path="/api/menu-items", format="json")
        self.assertEqual(response.status_code, 403)
    ...
    def test_menu_item_as_customer(self):
        ...
        response = self.client.get(path=f"/api/menu-items/{id}", format="json")
        self.assertEqual(response.status_code, 200)

        response = self.client.post(path=f"/api/menu-items/{id}", format="json")
        self.assertEqual(response.status_code, 405)

        response = self.client.put(path=f"/api/menu-items/{id}", format="json")
        self.assertEqual(response.status_code, 403)

        response = self.client.patch(path=f"/api/menu-items/{id}", format="json")
        self.assertEqual(response.status_code, 403)

        response = self.client.delete(path=f"/api/menu-items/{id}", format="json")
        self.assertEqual(response.status_code, 403)
