...
class UserGroupsTestCase(TestCase):
    def setUp(self):
        ...
        self.customer1 = User.objects.create_user(
            username="customer1@customer1.com", password="customer1"
        )
        self.customer2 = User.objects.create_user(
            username="customer2@customer2.com", password="customer2"
        )
        self.delivery1 = User.objects.create_user(
            username="delivery1@delivery1.com", password="delivery1"
        )
        self.delivery2 = User.objects.create_user(
            username="delivery2@delivery2.com", password="delivery2"
        )
        self.manager1 = User.objects.create_user(
            username="manager1@manager1.com", password="manager1"
        )
        self.manager2 = User.objects.create_user(
            username="manager2@manager2.com", password="manager2"
        )
    ...
    def test_manager_management(self):
        ...
        data = {"username": "customer1@customer1.com"}
        # add a new manager
        ...
        response = self.client.delete(path=f"/api/groups/manager/users/{id}", format="json")
    ...
    def test_delivery_management(self):
        ...
        data = {"username": "customer2@customer2.com"}
        # add a new delivery
        response = self.client.post(
        ...
        # access denied
        id = managers["results"][0]["id"]
        response = self.client.delete(path=f"/api/groups/delivery-crew/users/{id}", format="json")
        self.assertEqual(response.status_code, 403)
