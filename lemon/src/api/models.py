...
class Order(models.Model):
    ...
    def __str__(self):
        st_text = ""
        # status is nullable, so False and None are distinct states here and
        # `not self.status` would collapse them.
        if self.delivery is not None and self.status is False:
            st_text = "Out for delivery"
        elif not self.delivery and self.status is False:
